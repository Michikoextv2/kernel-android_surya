...
/* Prototipe entry ASM, agar genksyms/modversions bisa membuat CRC */
...
/* Enable akselerasi jika SIMD/NEON boleh dipakai */
static inline int lz4_decompress_accel_enable(void)
{
    return may_use_simd();
}

/* Array pointer ke fungsi ASM, dipilih per CPU */
extern int (*lz4_decompress_asm_fn[])(uint8_t **dst_ptr, uint8_t *dst_begin,
                                      uint8_t *dst_end, const uint8_t **src_ptr,
                                      const uint8_t *src_end, bool dip);

/* Wrapper inline: memanggil fungsi ASM dengan kernel_neon_begin/end */
static inline ssize_t lz4_decompress_asm(uint8_t **dst_ptr,
                                         uint8_t *dst_begin,
                                         uint8_t *dst_end,
...
#else /* !CONFIG_ARM64 || !CONFIG_KERNEL_MODE_NEON */

/* Fallback jika akselerasi tidak tersedia */
static inline int lz4_decompress_accel_enable(void)
{
    return 0;
}
...
#endif /* CONFIG_ARM64 && CONFIG_KERNEL_MODE_NEON */
